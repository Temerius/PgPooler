...
Отдельная PostgreSQL (в Docker — сервис **pgpooler-analytics**, порт 5434) для хранения:

- **connection_sessions** — история подключений (session_id, worker_id, client_addr, user, database, backend, pool_mode, application_name, connected_at, disconnected_at, duration_sec, disconnect_reason).
- **queries** — каждый запрос через пулер (connection_session_id, query_text, started_at, finished_at, duration_ms, command_type, rows_affected/returned, bytes, error).
- **query_fingerprints** — нормализованный текст для агрегации.
- **pool_snapshots** — снимки пулов (active/idle/waiting, avg_wait_ms) по времени.
- **events** — журнал событий (connect, disconnect, kill, block, error, config_reload).
- **blocked_users** — заблокированные пользователи (username, blocked_at, reason, unblocked_at).

Схема и индексы: **migrations/001_init_analytics_schema.sql**. Ядро пишет в аналитику через **AnalyticsWriter** (очередь + фоновый поток, libpq). Включение: в конфиге или в env: `analytics.enabled: true` / `PGPOOLER_ANALYTICS_ENABLED=1`, параметры `PGPOOLER_ANALYTICS_HOST`, `PGPOOLER_ANALYTICS_PORT`, и т.д. Точки вызова в сессии: [docs/ANALYTICS_INTEGRATION.md](docs/ANALYTICS_INTEGRATION.md).
